...
| API sirti | `/docs/staff` — 46 yo'l | `/docs/client` — 26 yo'l |
| Kirish | Telegram **bot** (parolsiz) | Telegram **bot** |
...
### Qurilmani ro'yxatdan o'tkazing — kirishdan keyin darhol

Push xabari **manzilsiz bo'lmasin**: server foydalanuvchi qaysi
telefonda ekanini faqat shu orqali biladi.

```
POST /api/v1/me/devices        {token, platform}     mijoz ilovasi
POST /api/v1/staff/devices     {token, platform}     xostes ilovasi
```

`platform`: `ios` · `android` · `web`. `token` — FCM/APNs tokeni.

```json
{ "id": "90d52818-3702-47c6-8b0e-832f183173f5",
  "platform": "ios",
  "last_seen_at": "2026-08-26T10:21:43.798720Z",
  "created_at": "2026-08-26T10:21:43Z" }
```

Uch qoida:

1. **Har ishga tushganda yuboring.** Takror yozuv paydo bo'lmaydi —
   ayni token qayta kelsa egasi va `last_seen_at` yangilanadi.
2. **Kirishdan keyin**, oldin emas. So'rov `401` bilan qaytadi va
   token yo'qoladi.
3. **Chiqishda o'chiring:**
   ```
   DELETE /api/v1/me/devices?token=...
   DELETE /api/v1/staff/devices?token=...
   ```
   Aks holda keyingi foydalanuvchining xabari shu telefonga tushadi.
   `logout` buni **o'zi qilmaydi** — token ilovada, serverda emas.

Yo'q tokenni o'chirish ham `204` qaytaradi, ya'ni chiqish oqimi xatoga
uchramaydi.

> Qurilma ro'yxatdan o'tmagan bo'lsa xabar **yo'qolmaydi** — u
> `GET /me/notifications` ro'yxatida turadi, shunchaki push kelmaydi.
...
| Smena yakuni | ✅ | `xostes/04-smena-yakuni.md` |
| Mehmon yaqinlashdi (geofence) | ❌ | — |
| Offline rejim | ❌ ilovada | backend tayyor, quyiga qarang |
...
Xostesdagi kabi **Telegram boti** — bitta bot ikkala ilovaga xizmat
qiladi. Farqi: mijozda ro'yxatdan o'tish ochiq, raqam tizimda bo'lmasa
yangi hisob yaratiladi (`is_new_user: true`).

```
POST /auth/telegram/start   → { nonce, deep_link }
deep_link ni oching         → Telegram, "Raqamni yuborish"
GET  /auth/telegram/status/{nonce}  → har 2 soniyada
      202 kutilmoqda · 200 token · 404 urinish eskirgan
```

Bot `request_contact` orqali **tasdiqlangan** raqamni beradi, ya'ni
`users.phone` birinchi kirishdayoq to'ladi. Ikki holatda `null` qoladi
(raqam +998 formatida emas yoki boshqa Telegram hisobida) — kirish
baribir davom etadi.

`POST /auth/telegram` (Login Widget) ham bor, lekin u **veb uchun**:
widget — domenga bog'langan iframe, native ilovada ishlamaydi va telefon
bermaydi. Mobil ilovada yuqoridagi oqimni ishlating.
...
2. **Qurilmani ro'yxatdan o'tkazish** (`POST /staff/devices`) — kirish
   oqimining bir qismi, keyinga qoldirmang
3. Muassasa tanlash (bir necha filialli xodim uchun)
4. **Bugun** ekrani — HOZIR / KEYINGI guruhlash klient tomonda
5. Bron detali va holat tugmalari (`allowed` ro'yxatini ishlating)
6. QR skanerlash
7. Offline navbat skeleti — UUID kalitlari bilan, **hozirdan**
8. Zal, Navbat
9. Smena yakuni

**Mijoz ilovasi**:

1. Telegram kirish + onboarding + qurilmani ro'yxatdan o'tkazish
...
# Mijoz: Telegram kirish — aynan shu tartibda
curl -X POST https://34-0-250-111.sslip.io/api/v1/auth/telegram/start
curl https://34-0-250-111.sslip.io/api/v1/auth/telegram/status/<NONCE>
