...
Bot mijoz ilovasi bilan **umumiy** (Telegramda bir botga bir webhook
to'g'ri keladi). Bu ilovaga ta'sir qilmaydi: `deep_link` ni `start`
javobidan oling, nomini o'zingiz yasamang.
...
Xostes ilovasining **butun backendi tayyor**:

| Ekran | Holat |
|---|---|
| Bugun (bronlar ro'yxati) | ✅ `02-bugun-va-qr.md` |
| Zal (stollar) | ✅ `frontend/veb-admin/02-zal-done.md` (API bir xil) |
| Navbat | ✅ `03-navbat.md` |
| QR skanerlash | ✅ `02-bugun-va-qr.md` |
| Smena yakuni | ✅ `04-smena-yakuni.md` |
| Profil | Qisman — `GET /staff/me` bor |

Qolgani faqat `Mehmon yaqinlashdi` (geofence) — u hali yo'q.

## 5.1. Kirgandan keyin: qurilmani ro'yxatdan o'tkazing

```
POST /api/v1/staff/devices     {"token": "...", "platform": "android"}
```

Push xabari shu tokenga boradi. Chiqishda o'chiring:

```
DELETE /api/v1/staff/devices?token=...
```

`logout` buni o'zi qilmaydi — token ilovada saqlanadi, serverda emas.
O'chirmasangiz keyingi xodimning xabari shu planshetga tushadi.
Batafsil: `../00-boshlash.md` §2.
